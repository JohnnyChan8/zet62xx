...
#define CONFIG_RK3128_BOARD_TEST
 ...
 */
//#define TS_RST_GPIO	        GPIO0_B4
//#define TS_INT_GPIO	        GPIO0_B0
...
#define MOUSE_REL_INPUT 1
#if (MOUSE_REL_INPUT == 1)
struct timer_list s_timer;
atomic_t counter;
int gesture = 0;
#define TIMER_TICK (HZ/20)
#define TICK_2_CLICK (3)

u32 x_bak=0, y_bak=0;
int x_rel=0, y_rel=0;
int first_press = 0;

static void mouse_timer_handle(unsigned long arg)
{
	int count;
	mod_timer(&s_timer, jiffies + TIMER_TICK);
	atomic_inc(&counter);
	
	count = atomic_read(&counter);
	//printk("%s: count = %d\n", __func__, count);
}
#endif
...
{
	gpio_direction_output(x_TS_RST_GPIO, 0);
}
...
{
	gpio_direction_output(x_TS_RST_GPIO, 1);
}
...
{
	if(gpio_get_value(x_TS_INT_GPIO) == 0)
	{
...
static void zet622x_ts_mouse_report(struct zet622x_tsdrv *ts)
{
	...
	{
		x = finger_report[0].report_x; 
		y = finger_report[0].report_y;
	...
	{
		//if (finger_report[0].valid != last_valid)
		{
			first_press = 0;
			x_rel = 0;
			y_rel = 0;
			count = atomic_read(&counter);
			printk("chenzy : count=%d\n", count);
			if (count<3)
			{
				gesture = 1;
				printk("chenzy : short click\n");
			}
			else
			{
				printk("chenzy : long click\n");
				gesture = 0;
			}
		}
	}
	...
	input_report_rel(ts->input, REL_Y, -y_rel);
	input_sync(ts->input);
}
...
static u8 zet622x_ts_parse_dynamic_finger(struct i2c_client *client)
{
	...
	//printk("ts_data print end\n");
	//printk("chenzy : valid = %X \n", valid);
	...
	for(i = 0; i < finger_num; i++)
	{
		...
		//x_temp = (u8)(ts_data[FINGER_HEADER_SHIFT+FINGER_PACK_SIZE*i]  )*256 + (u8)ts_data[(FINGER_HEADER_SHIFT+FINGER_PACK_SIZE*i)+1];
		//y_temp = (u8)(ts_data[FINGER_HEADER_SHIFT+FINGER_PACK_SIZE*i]+2)*256 + (u8)ts_data[(FINGER_HEADER_SHIFT+FINGER_PACK_SIZE*i)+3];
...
static void zet622x_ts_finger_report(struct zet622x_tsdrv *ts)
{
	...
	//printk("chenzy : %s -- finger_num=%d\n", __func__, finger_num);
	///-------------------------------------------///
	/// check have finger data
	///-------------------------------------------///
	for(i = 0; i < finger_num; i++)
	{
		//printk("chenzy :valid=%d, x=%d, y=%d\n", finger_report[i].valid, finger_report[i].report_x, finger_report[i].report_y);
		if(finger_report[i].valid == TRUE)
		{
			chk_finger = TRUE;
			finger_cnt = finger_cnt + 1;
			x = finger_report[i].report_x; 
			y = finger_report[i].report_y;
			
			//printk("chenzy : finger_report[%d]: x=%d, y=%d\n", i, x, y);
			break;//chenzy.
		}
	}
	...
	else
	{
		first_press = 1;
		input_report_key(ts->input, BTN_LEFT, 0);
		input_report_rel(ts->input, REL_X, 0);
                input_report_rel(ts->input, REL_Y, 0);
                input_sync(ts->input);
	}
	//need return;
	return;
...
static int  zet622x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	zet6221_ts->gpio = x_TS_INT_GPIO;
	
	printk("chenzy : TS_INT_GPIO:%d,TS_RST_GPIO:%d.\n",x_TS_INT_GPIO,x_TS_RST_GPIO);
...
*/	
	...
	{
		printk("chenzy : downloader error\n");
		while (1) {};
...
#if (MOUSE_REL_INPUT == 1)
	init_timer(&s_timer);
	s_timer.function = &mouse_timer_handle;
	s_timer.expires = jiffies +TIMER_TICK;
